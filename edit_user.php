...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Анхаар: Энэ утгууд яг баазын ENUM-тай таарах ёстой
    $role = $_POST['role']; 
    $status = $_POST['status']; 
    ...
    try {
        // Баазын бүтцийг дахин нягтлах (Role: admin, moderator, user)
        $sql = "UPDATE users SET username = ?, email = ?, role = ?, status = ?, full_name = ?, phone = ? WHERE id = ?";
        $stmt = $pdo->prepare($sql);
        
        // Debug хийхэд зориулж утгуудыг шалгах (Хөгжүүлэлтийн үед)
        // echo "Role: $role, Status: $status"; exit;

        $stmt->execute([$username, $email, $role, $status, $full_name, $phone, $user_id]);
        
        $message = "Хэрэглэгчийн мэдээлэл амжилттай шинэчлэгдлээ!";
        
        // Шинэчлэгдсэн мэдээллийг дахин татахын тулд refresh хийх эсвэл доорх query-г дахин ажиллуулна
    } catch (PDOException $e) {
        // "Data truncated" алдааг хэрэглэгчид ойлгомжтойгоор харуулах
        if (strpos($e->getMessage(), 'Data truncated') !== false) {
            $error = "Өгөгдлийн алдаа: Сонгосон 'Role' эсвэл 'Status' утга баазад таарахгүй байна. (" . $e->getMessage() . ")";
        } else {
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="role" id="role" class="w-full rounded-lg border-slate-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white transition-all">
                                        ...
                                        <option value="moderator" <?php echo $user['role'] == 'moderator' ? 'selected' : ''; ?>>Модератор (Moderator)</option>
                                ...
                                <div>
                                    ...
                                    <select name="status" id="status" class="w-full rounded-lg border-slate-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white transition-all">
                                        ...
                                        <option value="pending" <?php echo $user['status'] == 'suspended' ? 'selected' : ''; ?>>Хүлээгдэж буй (Pending)</option>
