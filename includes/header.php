...
// db.php-г олох зам (absolute path ашиглах нь найдвартай)
...
if (isset($_SESSION['user_id'])) {
    ...
    try {
        ...
        $stmt_count->execute([$loggedInUser['id']]);
        $unreadCount = $stmt_count->fetchColumn();
                                        ...
                                        <?php if (count($notifications) > 0): ?>
                                            <?php foreach ($notifications as $notif): ?>
                                                <?php 
                                                    ...
                                                    if ($notif['type'] == 'success') $icon = 'fas fa-check-circle text-green-500';
                                                    if ($notif['type'] == 'warning') $icon = 'fas fa-exclamation-triangle text-yellow-500';
                                                    if ($notif['type'] == 'error') $icon = 'fas fa-times-circle text-red-500';
                                                ?>
                        ...
                        <div class="relative group" id="userMenu">
                            ...
                            <div class="absolute right-0 top-full pt-2 w-56 hidden group-hover:block z-50">
                                <div class="bg-white rounded-xl shadow-xl border border-gray-100 py-2 animate-fade-in-down">
                                    ...
                                    <a href="profile/dashboard.php" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700 transition-colors">
                                        <i class="far fa-user w-4"></i> Хувийн хуудас
                                    </a>
                                    <a href="profile/wallet.php" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700 transition-colors">
                                        <i class="fas fa-wallet w-4"></i> Хэтэвч
                                    </a>
                                    
                                    <!-- Mobile/Tablet only links inside dropdown -->
                                    ...
                                    <a href="profile/settings.php" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700 transition-colors">
